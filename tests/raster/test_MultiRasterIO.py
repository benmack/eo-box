...
def test_MultiRasterIO__get_array__with_windows__native_resolution():
    ...
    mr.dst_res = None   
    ji_res_wins = mr.ji_windows(0)
    arrs = mr.get_arrays(ji_win=ji_res_wins) # only this line is different compared to the previous test
    assert isinstance(arrs, list) 
    assert len(arrs) == 3 
    assert len(arrs[0].shape) == 2 
    assert arrs[0].shape[0] == 128
...
def test_MultiRasterIO__process_window():
    ...
    mr.block_windows()
    result_arr_window = mr._process_window(0,
                                           func=process_ndvi,
...
def test_MultiRasterIO__process_windows():
    mr = MultiRasterIO(layer_files=layer_files, dst_res=20)
    mr.block_windows()
    result_arrs_windows = mr._process_windows(func=process_ndvi,
                                              idx_b04=0, idx_b08=1, idx_b8a=2)
...
def test_MultiRasterIO__process_windows_merge_stack():
    mr = MultiRasterIO(layer_files=layer_files, dst_res=20)
    mr.block_windows()
    result_arr = mr._process_windows_merge_stack(func=process_ndvi,
                                                 idx_b04=0, idx_b08=1, idx_b8a=2)
