...
def eocube_input_1(tmpdir):
    ...
    layers_df = (
        pd.Series([p.stem for p in layers_paths])
    ...
    layers_df["uname"] = (
        layers_df.sceneid.str[:3]
        + "_"
        + layers_df.date.dt.strftime("%Y-%m-%d")
        + "_"
        + layers_df.band.str[::]
    )
    ...
    input_kwargs = {"df_layers": layers_df_year, "tmpdir": tmpdir.mkdir("temp_dst_dir-0")}
    return input_kwargs
...
def eocube_input_onescene(tmpdir):
    ...
    input_kwargs = {"layers_df": layers_df, "tmpdir": a_tmpdir, "dst_paths": dst_paths}
    return input_kwargs
