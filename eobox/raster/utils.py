...
def cleanup_df_values_for_given_dtype(df, dtype, lower_as=None, higher_as=None, nan_as=None):
    ...
    dtype_range = dtype_ranges[dtype]
    lut = {"lower_as": lower_as, "higher_as": higher_as, "nan_as": nan_as}
    for key, value in lut.items():
        ...
        if isinstance(value, str):
            if value not in ["min_range", "max_range"]:
                raise ValueError(
                    f"'*_as' parameters must be numeric or in case of a string 'min_range' or 'max'range'."
                )
            else:
                if value == "min_range":
            ...
            if (value < dtype_range[0]) | (value > dtype_range[1]):
                raise ValueError(
                    "Numeric value of the '*_as' parameters must be in the data range of the given dtype."
                )
            else:
                lut[key] = value
        else:
